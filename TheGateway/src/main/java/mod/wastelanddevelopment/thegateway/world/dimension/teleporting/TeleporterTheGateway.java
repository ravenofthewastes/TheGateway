...
public class TeleporterTheGateway implements ITeleporter {
    protected final ServerWorld world;
    protected final Random random;

    public TeleporterTheGateway(ServerWorld worldIn) {
        this.world = worldIn;
    ...
    public Entity placeEntity(Entity entity, ServerWorld currentWorld, ServerWorld destWorld, float yaw, Function<Boolean, Entity> repositionEntity) {
        ...
        if (!placeInPortal(result, result.rotationYaw)) {
            BlockPos freePos = getFreePos(entity);
            entity.setPosition(freePos.getX(),freePos.getY(),freePos.getZ());
            makePortal(result);
            placeInPortal(result, result.rotationYaw);
        }
        return result;
    }
    ...
    private BlockPos getFreePos(Entity e){
        ...
        int posX = (int) e.getPosX();
        int posY = (int) e.getPosY();
        int posZ = (int) e.getPosZ();
        boolean foundPos = false;

        loop1:for(int x=-10;x<=10;x++){
            loop2:for(int y=-10;y<=10;y++){
                loop3:for(int z=-10;z<=10;z++){
                    BlockPos checkingPos = new BlockPos(posX+x,posY+y,posZ+z);
                    if(world.isAirBlock(checkingPos)){
                        if(world.isAirBlock(checkingPos.up())){
                            if(!world.isAirBlock(checkingPos.down())){
                                result=checkingPos;
                                break;
                            }
                        }
    ...
    public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_, double p_222272_6_, boolean p_222272_8_) {
        ...
        return optional.map((p_226707_7_) -> {
            ...
            BlockPattern.PatternHelper blockpattern$patternhelper = KathPortal.createPatternHelper(this.world, blockpos);
    ...
    public boolean makePortal(Entity entityIn) {
        ...
        BlockState blockstate = RegistryBlocks.KATH_PORTAL.get().getDefaultState().with(KathPortal.AXIS, l6 == 0 ? Direction.Axis.Z : Direction.Axis.X);
